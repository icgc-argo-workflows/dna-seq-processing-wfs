...
include SongPayloadUpload as SPU from "../modules/raw.githubusercontent.com/icgc-argo/data-processing-utility-tools/song-payload-upload.0.1.2.1/tools/song-payload-upload/song-payload-upload.nf" params(params)
...
workflow SeqExperimentUpload {
  ...
  main:
    ...
    SPU(song_url, payloadGenSeqExperiment.out.payload, token_file)

    songAnalysisGet(SPU.out.analysis_id, SPU.out.study, song_url, token_file)
    ...
    if (is_submission) {
      ...
      songAnalysisPublish(SPU.out.analysis_id, SPU.out.study, scoreUpload.out[0], song_url, token_file)
    }
